...
# Cette varible stocke les tuples dans les livres, dans le dictionnaire il y a des tu et les valeurs correspondantes, chaque tu étant une étude différente
# ID des livres, et la valeur correspondante est le nombre d'occurrences de ce mot dans le livre avec l'ID correspondant
tableIndexDataObject =   dict({"data": dict(),"status" : True})
loadingBack = dict({"status": True})

#Téléchargez des données pour les livres. 10 livres par groupe
#getTableIndex pour obtenir des données détaillées sur chaque livre, y compris des données sur chaque mot du livre et des informations détaillées sur le livre.
# loadingData() est responsable du chargement et du traitement des données liées aux livres, y compris les données sur chaque mot du livre et des informations détaillées sur chaque livre.
def loadingData():
...
def create_app(debug=True):
    ...
    def search_data():
        ...
        loadingData()
        time_start = time.time()
        #Cette fonction récupérera les mots-clés de recherche à partir des données JSON envoyées du côté client via request.json['word'].
        # Ce mot-clé est converti en minuscules (lowerWord) puis ajouté au dictionnaire historyWords.
        # Si le mot-clé existe déjà dans le dictionnaire, son nombre d'occurrences sera augmenté de 1.
        # Si le mot-clé n'existe pas, il sera ajouté au dictionnaire avec un nombre d'occurrences de 1.
